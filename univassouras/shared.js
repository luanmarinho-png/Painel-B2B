...
window.CURRENT_INSTITUTION = CURRENT_INSTITUTION;
 ...
 try {
  ...
  root.innerHTML = `
    ...
    <section class="section-shell" style="margin-bottom:24px">
      <div style="display:flex;flex-wrap:wrap;gap:10px;padding:14px 16px;border:2px solid #dc2626;border-radius:16px;background:var(--bg-card);box-shadow:0 4px 18px rgba(220,38,38,0.12)">
        ${heroPills}
      </div>
    </section>
...
// ── Exportar PDF / Excel institucional (mesmo fluxo do admin: _simOpenPdfModal → _simGeneratePDF + _gerarRelatorioExcel) ──
/**
 * Carrega assets/coord-institutional-report.js (espelho do admin.html).
 * @returns {Promise<void>}
 */
function _coordEnsureInstitutionalReportScript() {
  if (typeof window.coordExportInstitutionalPdf === "function") return Promise.resolve();
  return new Promise((resolve, reject) => {
    const s = document.createElement("script");
    s.src = "/assets/coord-institutional-report.js";
    s.onload = () => resolve();
    s.onerror = () => reject(new Error("coord-institutional-report"));
    document.head.appendChild(s);
  });
}

window._exportSimPDF = async function () {
  try {
    await _coordEnsureInstitutionalReportScript();
    await window.coordExportInstitutionalPdf();
  } catch (e) {
    console.error("[Export PDF institucional]", e);
    alert("Não foi possível gerar o relatório PDF. Atualize a página ou tente novamente.");
  }
};

window._exportSimExcel = async function () {
  try {
    await _coordEnsureInstitutionalReportScript();
    await window.coordExportInstitutionalExcel();
  } catch (e) {
    console.error("[Export Excel institucional]", e);
    alert("Não foi possível gerar a planilha Excel. Atualize a página ou tente novamente.");
  }
};
